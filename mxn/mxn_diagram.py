...
class MxNDiagram(MxNTreeNode):
    ...
    def _get_eps_tu(self):
        if len(self.cs.reinf_components_with_state) == 1 and \
            self.cs.reinf_components_with_state[0].__class__ == RLCTexUniform:
            eps_tex_u = self.cs.reinf_components_with_state[0].ecb_law.eps_u
            # @todo: assumptions on epsilon profile unclear. Is the cross section
            # ina state of pure bending?
            return self.cs.reinf_components_with_state[0].convert_eps_tex_u_2_lo(eps_tex_u)
        eps = 0
    ...
    def _get_eps_range(self):
        eps_ttu = self.cs.reinf_components_with_state[0].ecb_law.eps_u

        eps_c_space = np.linspace(self.eps_cu, 0, self.n_eps)
        eps_t_space = np.linspace(0, eps_ttu, self.n_eps)
        ...
        eps_tt = np.linspace(self.eps_tu, eps_ttu, self.n_eps)
